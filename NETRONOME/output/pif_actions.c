...
static int pif_action_exec_ingress__act_forward(__lmem uint32_t *_pif_parrep, __xread uint32_t *_pif_actdatabuf, unsigned _pif_debug)
{
    int _pif_return = PIF_RETURN_FORWARD;
    __xread struct pif_action_actiondata_ingress__act_forward *_pif_act_data = (__xread struct pif_action_actiondata_ingress__act_forward *)_pif_actdatabuf;
...
#endif
        __debug_label("pif_table_hit_ingress__act_forward");
        local_csr_write(local_csr_mailbox_2, temp0);
        local_csr_write(local_csr_mailbox_3, temp1);
    }
#endif
#ifdef PIF_DEBUG
    __debug_label("pif_action_state_ingress__act_forward");
#endif
...
extern __forceinline int pif_action_exec_op(__lmem uint32_t *parrep, __xread uint32_t *_actdata)
{
    ...
    case PIF_ACTION_ID_ingress__act_forward:
        ret = pif_action_exec_ingress__act_forward(parrep, _actdata + PIF_ACTION_OPDATA_LW, opdata->dbg_flags);
        break;
    }
#ifdef PIF_DEBUG
